    ...
    <div className="flex flex-col min-h-screen bg-black text-white font-sans overflow-x-hidden">
      ...
      <section className="relative w-full py-16 md:py-24 px-6 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        ...
        <div className="lg:col-span-5 flex justify-center items-center">
          <div className="relative w-[230px] sm:w-[250px] md:w-[270px] aspect-[9/19] rounded-[38px] border-[6px] border-zinc-800 bg-zinc-950 overflow-hidden shadow-[0_0_40px_rgba(197,242,54,0.15)] ring-1 ring-zinc-700/50">
            {/* Phone Notch/Island */}
      ...
      {/* Section Header */}
      <section className="bg-black px-6 pt-24 pb-8">
        <div className="max-w-4xl mx-auto text-center space-y-5">
          <span className="text-sm font-semibold text-[#C5F236] tracking-widest uppercase">The Alpha Terminal</span>
      ...
      <section className="bg-black px-6 py-16 space-y-32">
            ...
            <div key={flow.id} className="max-w-6xl mx-auto">
              <div className={`grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center ${!isEven ? 'lg:[direction:rtl]' : ''}`}>
                ...
                <div className={!isEven ? 'lg:[direction:ltr]' : ''}>
                  <span className="text-[#C5F236] text-sm font-bold tracking-widest uppercase">
                    {index + 1}
                  </span>
                  <h3 className="text-2xl md:text-4xl font-black tracking-tight text-white leading-tight mt-3">
                    {flow.title}
                  </h3>
                  <p className="text-sm md:text-base text-white/45 leading-relaxed mt-4 max-w-md">
                    {flow.desc}
                  </p>
                  <ul className="mt-6 space-y-3">
                    {['Auto-routing via Jupiter', 'MEV protected transactions', 'Mainnet live'].map((item) => (
                      <li key={item} className="flex items-center gap-3 text-sm text-white/60">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#C5F236] flex-shrink-0" />
                        {item}
                      </li>
                    ))}
                  </ul>
                ...
                {/* Screenshot */}
                <div className={!isEven ? 'lg:[direction:ltr]' : ''}>
                  <div className="relative bg-zinc-900/60 border border-white/5 rounded-2xl p-5 md:p-8 overflow-hidden shadow-lg">
                    <img
                      src={flow.image}
                      alt={flow.title}
      ...
      <section className="py-24 bg-zinc-950 border-t border-white/5 relative overflow-hidden">
        <div className="max-w-6xl mx-auto px-6 mb-16 text-center">
          <span className="text-[#C5F236] text-xs font-bold tracking-widest uppercase px-3 py-1.5 rounded-full bg-[#C5F236]/10 border border-[#C5F236]/20">
            Interface Tour
          </span>
        ...
        <div className="block md:hidden">
          <div className="flex justify-center gap-4 mb-6 z-30 relative">
            <button
              ...
              className="w-10 h-10 flex items-center justify-center bg-zinc-900 border border-white/10 rounded-full hover:border-[#C5F236] hover:text-[#C5F236] transition-all"
            >
              ←
            </button>
            <button
            ...
            >
              {APP_SCREENS.map((screen, idx) => {
                ...
                return (
                  <div
                    key={screen.id}
                    className="absolute inset-0 rounded-[20px] border-[2px] border-zinc-800 bg-zinc-950 overflow-hidden shadow-2xl backface-hidden"
                    style={{
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent flex flex-col justify-end p-3">
                      <span className="text-[8px] font-bold text-[#C5F236] uppercase tracking-wider">
                        {screen.label}
                      </span>
                      <h4 className="text-[10px] font-bold text-white mt-0.5">{screen.title}</h4>
                    </div>
                  </div>
                )
              })}
            ...
            {APP_SCREENS.map((screen, idx) => {
              ...
              return (
                <div
                  ...
                  className={`relative rounded-3xl border border-white/5 bg-zinc-900/20 p-6 overflow-hidden flex gap-6 justify-between items-center transition-all duration-500 ease-out cursor-pointer ${gridClasses} ${
                    isDimmed ? 'opacity-35 scale-[0.97] blur-[1px]' : 'opacity-100 scale-100'
                  } ${isHovered ? 'border-[#C5F236]/50 shadow-[0_0_40px_rgba(197,242,54,0.12)] bg-zinc-900/60' : ''}`}
                >
                  <div className="flex-1 flex flex-col h-full justify-between z-10">
                    <div>
                      <span className="text-[10px] font-bold text-[#C5F236] uppercase tracking-wider">
                        {screen.label}
                      </span>
                      <h3 className="text-xl font-black text-white mt-1 leading-tight">{screen.title}</h3>
                    </div>
                    <p className="text-xs text-white/45 leading-relaxed mt-2 max-w-[260px]">
                      {screen.desc}
                    </p>
                  </div>

                  <div className="relative flex-shrink-0 flex items-center justify-center overflow-hidden rounded-xl border border-white/10 bg-black w-[110px] aspect-[9/19.5]">
                    <img
                      src={screen.image}
                      alt={screen.title}
